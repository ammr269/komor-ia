...
export default function ApiKeysPage() {
  ...
  const [models, setModels] = useState([]) // ✅ État pour les modèles
  const [loading, setLoading] = useState(true)
  const [loadingModels, setLoadingModels] = useState(true) // ✅ Loading modèles
  ...
  useEffect(() => {
    fetchApiKeys()
    fetchModels() // ✅ Charger les modèles au démarrage
  }, [])
  ...
  const createApiKey = async (e) => {
    ...
    try {
      const response = await fetch('/api/user/api-keys', {
        ...
        body: JSON.stringify({
          name: newKeyName,
          modeleId: selectedModel === 'all' ? null : selectedModel,
        }),
      })
      ...
      if (response.ok) {
        setNewKeyGenerated(data.apiKey.key) // ✅ Récupérer la clé depuis apiKey.key
        setNewKeyName('')
              ...
              {newKeyGenerated ? (
                ...
                <form onSubmit={createApiKey}>
                  ...
                  <div className="mb-6">
                    ...
                    >
                      <option value="all">
                        {loadingModels ? 'Chargement...' : 'Tous les modèles'}
                      </option>
                      {/* ✅ Charger dynamiquement les modèles */}
                      {models.map((model) => (
                        <option key={model.id} value={model.id}>
                          {model.name}{' '}
                          {model.status !== 'production' && `(${model.status})`}
                        </option>
                      ))}
                    </select>
                  </div>
